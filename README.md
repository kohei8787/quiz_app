...
参加者・スクリーン表示・管理画面を分けて、問題の出題から回答公開・結果発表まで進行できます。
...
5. 次の問題へ進む、または **結果発表**（管理者が 3位→2位→1位を順に発表）→ **イベント終了**
...
flowchart TD
  ...
  subgraph StatusFlow [イベント進行]
    ...
    Results["結果発表<br/>results_announced"]
    ...
    Correct -->|結果発表| Results
    Survey -->|結果発表| Results
    Ranking -->|結果発表| Results
    Results -->|3位→2位→1位を発表| Results
    Results -->|イベント終了| Finished
...
| `ranking_revealed` | 順位一覧 | 順位一覧 |
| `results_announced` | 結果発表（1〜3位を順次表示） | 結果発表（1〜3位を順次表示） |
| `finished` | 最終順位＋出題レビュー | 終了＋最終順位 |

補足:

- **例題**（`question` → `answer_closed` → `started`）では回答公開以降のステップはありません
- **順位発表**は問題ごとの順位一覧、**結果発表**は最終の上位表彰です
- **結果発表**は、正解発表・アンケート結果・順位発表のいずれからでも遷移できます
- 結果発表画面では、管理者が **「3位を発表」→「2位を発表」→「1位を発表」** の順で手動公開します（会場の進行に合わせられます）
- **イベント終了**は結果発表のあとに進む想定です（参加受付中・終了済み以外からも実行できます）
- 管理画面はログイン後、現在のステータスに応じた操作ボタンだけが表示されます
- 誤操作防止のため、次の操作では確認ダイアログが出ます
  - **回答受付終了**（「回答受付を終了しますか？」）
  - **順位発表**（「現在の順位を表示しますか？」）
  - **イベント終了** / **新規イベント（リセット）**
...
5. 回答後、「回答受付終了」→「回答公開」→「正解発表」→「アンケート結果公開」→「順位発表」の順で操作する  
   （「回答受付終了」「順位発表」では確認ダイアログが出ます）  
   締めくくりは「結果発表」→「3位を発表」→「2位を発表」→「1位を発表」→「イベント終了」
...
- 単一選択肢に加えて、複数選択肢を対象にした問題も作成できます
  - 例: `"questionText": "{{A}}もしくは{{B}}と答えた人の割合は何％でしょう？"`
  - 例: `"focusOptions": ["A", "B"]`（A と B の両方をハイライト表示）
  - 互換性のため、従来の `"focusOption": "A"` もそのまま使えます
- イベント終了後、参加者画面には出題済み問題と自チームの回答レビューが表示されます
